...
LSM6DSOX::LSM6DSOX(hal::I2C::SharedPtr i2c, I2CAddr addr) : i2c_(i2c), i2c_addr_(addr) {
...
bool LSM6DSOX::setAccelUserOffset(int8_t x, int8_t y, int8_t z) {
    return writeReg(kXOfsUsr, static_cast<RegVal>(x)) &&
           writeReg(kYOfsUsr, static_cast<RegVal>(y)) && writeReg(kZOfsUsr, static_cast<RegVal>(z));
}
