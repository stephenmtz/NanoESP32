    ...
    for (;;) {
        if (xQueueReceive(gpio_evt_queue, &io_num, portMAX_DELAY)) {
            ...
            if (imu->readAccel(x, y, z)) {
                ESP_LOGI(TAG, "raw[%6d %6d %6d]  mg[%8.1f %8.1f %8.1f]", x, y, z,
                         imu->lsm6soxzFromFs2ToMg(x), imu->lsm6soxzFromFs2ToMg(y),
...
void setupAccelInterrupt(drivers::LSM6DSOX& imu) {
    ...
    gpio_isr_handler_add(kAccelInt1Pin, gpio_isr_handler, reinterpret_cast<void*>(kAccelInt1Pin));
}
